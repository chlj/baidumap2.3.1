...
/**
 * 18 百度地图搜索显示 + 标记多个点（不去除原有标记）_2
 * 
 * @author Administrator
 * 
 */

public class baidu_map_jiemian_search_show_more_2_Activity extends Activity {
	...
	MyLocationMapViewXYZVB mMapView = null; // 地图View
	...
	private List<baidu_map_point_demo> list_point_result = null; // 所有的点
	private List<String> list_select = null;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.baidumap_activity_jiemian_search_show_more2);

		list_point_result = getAllMarkPointList();// 测试使用
		...
		viewCache_dw = baidu_map_jiemian_search_show_more_2_Activity.this
				.getLayoutInflater().inflate(R.layout.act_paopao2, null); // 定位时
																			// 弹出的图层
		viewCache_dw.setTag("0");

		// 地图初始化
		mMapView = (MyLocationMapViewXYZVB) findViewById(R.id.bmapView);
		...
		// 自定义图标
		Drawable marker = baidu_map_jiemian_search_show_more_2_Activity.this
		...
		central_point.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Toast.makeText(
						baidu_map_jiemian_search_show_more_2_Activity.this,
						"开启定位", Toast.LENGTH_SHORT).show();

				
				map_check_txt_fj.setChecked(false);
				map_check_txt_pcs.setChecked(false);
				map_check_txt_jws.setChecked(false);
				map_check_txt_xz.setChecked(false);
		...
		// 派出所
		map_check_txt_pcs.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (map_check_txt_pcs.isChecked()) {
					list_select.add("1");

				} else {
					list_select.remove("1");
				}
				initOverlay();
		...
		map_check_txt_xz.setOnClickListener(new OnClickListener() {
	...
	public void createPaopao(final GeoPoint point, String value) {
		...
		viewCache = baidu_map_jiemian_search_show_more_2_Activity.this
		...
		MyLocationMapViewXYZVB.pop = pop;
		MyLocationMapViewXYZVB.mPopView = viewCache;
		...
		public void onGetPoiResult(MKPoiResult res, int type, int error) {
			...
			if (res.getCurrentNumPois() > 0) {
				...
				for (int i = 0; i < count; i++) {
					...
					Log.i("xx","搜索得到的name="+res.getAllPoi().get(i).name+",uid="+res.getAllPoi().get(i).uid);
	 ...
	 * @return
	 */
	public List<baidu_map_point_demo> getAllMarkPointList() {
		...
		// List<baidu_map_point_demo> tmpList = Linq4j
		// .asEnumerable(list)
		// .where(new Predicate1<baidu_map_point_demo>() {
		// @Override
		// public boolean apply(baidu_map_point_demo arg0) {
		// return arg0.getType() == "1";
		// }
		// })
		// .select(new Function1<baidu_map_point_demo, baidu_map_point_demo>() {
		//
		// @Override
		// public baidu_map_point_demo apply(baidu_map_point_demo arg0) {
		// // TODO Auto-generated method stub
		// return arg0;
		// }
		// }).toList();
		// Log.i("xx", "tmpList.size()=" + tmpList.size());
	...
	public void initOverlay() {
		...
		List<baidu_map_point_demo> showList = new ArrayList<baidu_map_point_demo>();
		if (list_point_result != null && list_point_result.size() > 0) {
			if (list_select != null && list_select.size() > 0) {

				for (int i = 0; i < list_select.size(); i++) {
					final int m = i;
					List<baidu_map_point_demo> tmpList = Linq4j
							.asEnumerable(list_point_result)
							.where(new Predicate1<baidu_map_point_demo>() {
								@Override
								public boolean apply(baidu_map_point_demo arg0) {
									return arg0.getType() == list_select.get(m);
								}
							})
							.select(new Function1<baidu_map_point_demo, baidu_map_point_demo>() {

								@Override
								public baidu_map_point_demo apply(
										baidu_map_point_demo arg0) {
									// TODO Auto-generated method stub
									return arg0;
								}
							}).toList();
		...
		if (showList != null && showList.size() > 0) {
			Log.i("xx", "showList=" + showList.size());
			for (int i = 0; i < showList.size(); i++) {
				baidu_map_point_demo model = showList.get(i);
			...
			GeoPoint point = showList.get(0).getPoint(); // (经度,纬度)
 ...
 * 
 */
class MyLocationMapViewXYZVB extends MapView {
	static PopupOverlay pop = null;// 弹出泡泡图层，点击图标使用
	static View mPopView = null;// 弹出泡泡图层，点击图标使用

	public MyLocationMapViewXYZVB(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
	}

	public MyLocationMapViewXYZVB(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
