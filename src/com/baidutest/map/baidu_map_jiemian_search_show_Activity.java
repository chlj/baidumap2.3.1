...
public class baidu_map_jiemian_search_show_Activity extends Activity {
	...
	MyLocationMapViewXYZVS mMapView = null; // 地图View
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.baidumap_activity_jiemian_search_show);
		...
		viewCache_dw = baidu_map_jiemian_search_show_Activity.this
				.getLayoutInflater().inflate(R.layout.act_paopao2, null); // 定位时
																			// 弹出的图层
		viewCache_dw.setTag("0");

		// 地图初始化
		mMapView = (MyLocationMapViewXYZVS) findViewById(R.id.bmapView);
		...
		// 自定义图标
		Drawable marker = baidu_map_jiemian_search_show_Activity.this
		...
		central_point.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Toast.makeText(baidu_map_jiemian_search_show_Activity.this,
				...
				mLocClient = new LocationClient(
						baidu_map_jiemian_search_show_Activity.this);
				locData = new LocationData();
				mLocClient.registerLocationListener(myListener);
				LocationClientOption option = new LocationClientOption();
				option.setOpenGps(true);// 打开gps
	...
	public void createPaopao(final GeoPoint point, String value) {
		...
		viewCache = baidu_map_jiemian_search_show_Activity.this
		...
		img.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(baidu_map_jiemian_search_show_Activity.this,
		...
		MyLocationMapViewXYZVS.pop = pop;
		MyLocationMapViewXYZVS.mPopView = viewCache;
	...
	public class MySearchListener implements MKSearchListener {
		...
		public void onGetPoiResult(MKPoiResult res, int type, int error) {
			...
			// 错误号可参考MKEvent中的定义
			if (error != 0 || res == null) {
				Toast.makeText(baidu_map_jiemian_search_show_Activity.this, "抱歉，未找到结果",
			...
			if (res.getCurrentNumPois() > 0) {
				// 2. 自定义图标 将结果显示到地图上面
				Drawable marker = baidu_map_jiemian_search_show_Activity.this.getResources()
				...
				strInfo += "找到结果";
				Toast.makeText(baidu_map_jiemian_search_show_Activity.this, strInfo,
						Toast.LENGTH_LONG).show();
			}
	...
	class OverlayTestA extends ItemizedOverlay<OverlayItem> {
		...
		protected boolean onTap(final int index) {
			// 在此处理item点击事件
           //  createPaopao(arraylist.get(index).getPoint(), arraylist.get(index).getSnippet());

			View mPopView = getLayoutInflater().inflate(R.layout.act_paopao2, null);
			...
			img.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					intent.putExtra("toName", popupText.getText().toString()); // 目的地名称
					intent.putExtra("toLon", String.valueOf(arraylist.get(index).getPoint().getLongitudeE6() / 1e6));// 目的地经度
					intent.putExtra("toLat", String.valueOf(arraylist.get(index).getPoint().getLatitudeE6() / 1e6 ));// 目的地纬度
 ...
 * 
 */
class MyLocationMapViewXYZVS extends MapView {
	static PopupOverlay pop = null;// 弹出泡泡图层，点击图标使用
	static View mPopView = null;// 弹出泡泡图层，点击图标使用

	public MyLocationMapViewXYZVS(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
	}

	public MyLocationMapViewXYZVS(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
